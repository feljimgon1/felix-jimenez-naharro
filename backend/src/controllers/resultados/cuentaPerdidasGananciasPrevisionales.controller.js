...
async function gastosFinancierosM(req, res) {
    const balance = await getBalance(req, res);
...
async function resultadosOrdinariosAntesImpuestosM(req, res) {
    ...
    for (let i = 0; i < resultadosFinancieros.length; i++) {
        if (isNaN(resultadosFinancieros[i])) {
            resultadosFinancieros[i] = 0
        }
    }
...
async function impuestosSobreSociedadesM(req, res) {
    ...
    for (let i = 0; i < 5; i++) {
        if (resultadosOrdinariosAntesImpuestos[i] > 0) {
            impuestosSobreSociedades[i] = (resultadosOrdinariosAntesImpuestos[i] * estrategiaCirculante.tasaImpositiva) / 100
        }
        else {
...
async function resultadoDelEjercicioM(req, res) {
    ...
    resultadoDelEjercicio = [
        resultadosOrdinariosAntesImpuestos[0] - impuestosSobreSociedades[0],
        resultadosOrdinariosAntesImpuestos[1] - impuestosSobreSociedades[1],
        resultadosOrdinariosAntesImpuestos[2] - impuestosSobreSociedades[2],
        resultadosOrdinariosAntesImpuestos[3] - impuestosSobreSociedades[3],
        resultadosOrdinariosAntesImpuestos[4] - impuestosSobreSociedades[4],
    ]
...
cuentaPerdidasGananciasPrevisionalesController.getResultados = async (req, res) => {
    ...
    let cuentaPerdidasGananciasPrevisionales = [
        ...
        resultadosOrdinariosAntesImpuestos,
        impuestosSobreSociedades,
        resultadoDelEjercicio,
        autoFinanciacion
    ]
    ...
    for(let i = 0; i<cuentaPerdidasGananciasPrevisionales.length; i++){
        result_i = []
        for(let j = 0; j<cuentaPerdidasGananciasPrevisionales[i].length; j++){
            if(isNaN(cuentaPerdidasGananciasPrevisionales[i][j])){
                cuentaPerdidasGananciasPrevisionales[i][j] = 0
            }
            result_i.push(cuentaPerdidasGananciasPrevisionales[i][j])
        }
        result.push(result_i)
    }

    const user = await User.findById(req.user_id)
    const cuentaId = await user.cuentaPerdidasGananciasPrevisionalesId

   await CuentaPerdidasGananciasPrevisionales.findByIdAndUpdate(cuentaId, {
        ...
        resultadoEjercicio: resultadoDelEjercicio,
        autoFinanciacion: autoFinanciacion
    })

    let cpg= await (CuentaPerdidasGananciasPrevisionales.findById(user.cuentaPerdidasGananciasPrevisionalesId));

    console.log(cpg.importeNetoCifraVentas);

    res.json({
        cuentaPerdidasGananciasPrevisionales: cuentaPerdidasGananciasPrevisionales
    })

};


module.exports = cuentaPerdidasGananciasPrevisionalesController